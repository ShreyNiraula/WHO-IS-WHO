...
router.get('/:cid', (req, res, next)=>{
    ...
    const {currMinistry} = req.query

    // simply get the candidate info from all other tables joined
    const sql = `SELECT * FROM Candidate AS c NATURAL JOIN Position AS p NATURAL JOIN Designation AS d WHERE c.Candidate_id=${cid}`
    db.query(sql, (err, candidateInfo)=>{
      ...
      db.query(sql2, (err, historyInfo)=>{
        ...
        res.render('candidate', {candidateInfo:candidateInfo, historyInfo:historyInfo, currMinistry:currMinistry})
      })
