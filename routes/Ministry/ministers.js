...
router.get("/:id", (req, res, next) => {
  ...
  db.query(sql1, (err, isLogged)=>{
    ...
    let sql2 = `SELECT * FROM Candidate NATURAL JOIN Position Natural Join Designation where Position.Ministry_id = '${id}' order by Designation.Hierarchy asc`;
    db.query(sql2, (err, result) => {
      if (err) throw err;
      res.render("minister", { result: result, isLogged: isLogged });
  });
  })
});
